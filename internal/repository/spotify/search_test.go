...
func Test_outbound_Search(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func(args args) {
				...
				mockHTTPClient.EXPECT().Do(req).Return(&http.Response{
					StatusCode: 200,
					Body:       io.NopCloser(bytes.NewBufferString(searchResponseStr)),
				}, nil)
			},
		},
		{
			name: "ERROR: fail to decode search response",
			args: args{
				query:  "bohemian rhapsody",
				limit:  10,
				offset: 0,
		...
		},
		{
			...
			mockFn: func(args args) {
				...
				req.Header.Set("Authorization", "Bearer accessToken")
				mockHTTPClient.EXPECT().Do(req).Return(nil, fmt.Errorf("network error"))
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := o.Search(context.Background(), tt.args.query, tt.args.offset, tt.args.limit)
